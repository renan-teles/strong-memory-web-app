...
export class RegisterApiFacade {
  ...
  registerAdministrator(data: RegisterUserRequest): void {
    this.handleRegister(this.api.registerAdmin(data));
  }

  registerPlayer(data: RegisterUserRequest): void {
    this.handleRegister(this.api.registerPlayer(data));
  }

  private handleRegister(request$: Observable<ApiResponse<CreatedUserResponse>>): void {
