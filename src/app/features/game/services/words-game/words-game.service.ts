...
export class WordsGameService {
  private words: IWordData[] = [];
  private counter: number = 0;

  private readonly _currentWords = signal<IWordData[]>([]);
  currentWords: Signal<IWordData[]> = this._currentWords.asReadonly();

  private readonly _userWords = signal<string[]>([]);
  userWords: Signal<string[]> = this._userWords.asReadonly();

  setWords(words: IWordData[]): void {
    this.words = words;
  }
